<x-filament-panels::page>
    <div class="space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <select wire:model.live="filterClient" class="h-9 text-sm border border-zinc-200 dark:border-zinc-700 rounded-md bg-white dark:bg-zinc-900 px-3">
                <option value="all">Todos los clientes</option>
                @foreach($this->getClientsWithMedia() as $client)
                    <option value="{{ $client['id'] }}">{{ $client['name'] }}</option>
                @endforeach
            </select>

            <select wire:model.live="filterType" class="h-9 text-sm border border-zinc-200 dark:border-zinc-700 rounded-md bg-white dark:bg-zinc-900 px-3">
                <option value="all">Todos los tipos</option>
                <option value="portfolio">Portfolio</option>
                <option value="posts">Posts</option>
                <option value="library">Library uploads</option>
            </select>
            ...
            <div class="flex-1 max-w-sm ml-auto">
                <input
                    ...
                    class="w-full h-9 px-4 rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
                />
            </div>
        </div>
        ...
        <div class="flex gap-6">
                            ...
                            @foreach($media as $item)
                                ...
                                >
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="text-xs text-zinc-500">{{ $this->getOwnerName($item) }} · {{ $this->getOwnerContext($item) }}</div>
                                    </div>
                                    <div class="text-xs text-zinc-400">{{ $item->created_at->format('d/m/Y') }}</div>
                                </div>
                            @endforeach
                        </div>
                    @endif
            ...
            @if($selectedMediaId && ($selected = $this->getSelectedMedia()))
                <div class="w-80 shrink-0 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-4 self-start sticky top-4">
                    <div class="text-sm font-semibold text-zinc-900 dark:text-white">Detalles</div>
                    ...
                    <div class="space-y-2 text-xs text-zinc-500">
                        ...
                        <div><span class="font-medium text-zinc-700 dark:text-zinc-300">Cliente:</span> {{ $this->getOwnerName($selected) }}</div>
                        <div><span class="font-medium text-zinc-700 dark:text-zinc-300">Contexto:</span> {{ $this->getOwnerContext($selected) }}</div>
